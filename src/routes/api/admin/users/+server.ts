...
const mandatoryPayload = ['name', 'email', 'role'];

const allowedKeys = new Set([...mandatoryPayload, ...optionalPayload]);

const checkUserCreationPayload = (data: unknown): data is User => {
...
export const POST: RequestHandler = async ({ request, locals }) => {
    ...
    try {
        ...
        console.warn('POST /api/admin/users: invalid JSON body', (err as Error).message);
        return new Response(JSON.stringify({ error: 'Invalid JSON body' }), { status: 400 });
    }
    if (!data) {
        return new Response(JSON.stringify({ error: 'Invalid payload' }), { status: 400 });
    }
    const checkedData = checkUserCreationPayload(data);
    if (!checkedData) {
        return new Response(JSON.stringify({ error: 'Invalid payload' }), { status: 400 });
    }
